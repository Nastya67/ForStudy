...
void readPaus(struct playLists *pointPlaylist){
    char doc[] = "Memoryp1.txt";
    const char * toRead = &doc;
    char * r = "rb";
    FILE * memory;
    memory = fopen(doc, r);
    ...
    while(j < 20){
        ...
        while(i < 100 && lll == 0){
            for(k = 0; k <100; k++){
                ...
                pointPlaylist->someSong[i].pause[k] = atof(tmp2);
            }
            i++;
        }
        pointPlaylist++;
...
void savePaus(struct playLists *pointPlaylist){
    char doc[] = "Memoryp1.txt";
    const char * toWrite = &doc;
    char * w = "wb";
    FILE * memory;
    ...
    for(j = 0; j < 20; j++, pointPlaylist++){
        ...
        for(i = 0; i < 20; i++){
            for(k = 0; k < 100; k++){
                fprintf(memory, "%.0f\n", pointPlaylist->someSong[i].pause[k]);
            }
            fprintf(memory, ")\n");
        }
        fprintf(memory, "}\n");
